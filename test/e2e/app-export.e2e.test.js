...
FRODO_MOCK=record FRODO_HOST=https://openam-volker-demo.forgeblocks.com/am frodo app export -SNi HRLite -D appExportTestDir1
...
FRODO_MOCK=record FRODO_HOST=https://openam-volker-demo.forgeblocks.com/am frodo app export -SNaD appExportTestDir2
FRODO_MOCK=record FRODO_HOST=https://openam-volker-demo.forgeblocks.com/am frodo app export -A
FRODO_MOCK=record FRODO_HOST=https://openam-volker-demo.forgeblocks.com/am frodo app export --all-separate --sort --no-metadata --no-deps --directory appExportTestDir3
*/
...
describe('frodo app export', () => {
  ...
  test.skip('"frodo app export -SNi HRLite -D appExportTestDir1": should export the app with app id "HRLite" into the directory appExportTestDir1', async () => {
    const exportDirectory = 'appExportTestDir1';
    const CMD = `frodo app export -SNi HRLite -D ${exportDirectory}`;
  ...
  // TODO: Generate mocks for this test (skip for the meantime)
  test.skip('"frodo app export -SNaD appExportTestDir2": should export all apps to a single file in the directory appExportTestDir2', async () => {
    const exportDirectory = 'appExportTestDir2';
    const CMD = `frodo app export -SNaD ${exportDirectory}`;
    await testExport(CMD, env, type, undefined, exportDirectory, false);
  });
  ...
  // TODO: Generate mocks for this test (skip for the meantime)
  test.skip('"frodo app export --all-separate --sort --no-metadata --no-deps --directory appExportTestDir3": should export all apps to separate files in the directory appExportTestDir3', async () => {
    const exportDirectory = 'appExportTestDir3';
    const CMD = `frodo app export --all-separate --sort --no-metadata --no-deps --directory ${exportDirectory}`;
    await testExport(CMD, env, type, undefined, exportDirectory, false);
  });
});
