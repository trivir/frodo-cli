...
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo mapping export -xi sync/extract_script_test -D mappingExportTestDir8
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo mapping export -i mapping/mappingTest --extract -D mappingExportTestDir9
*/
...
describe('frodo mapping export', () => {
    ...
    test('"frodo mapping export -xi sync/extract_script_test -D mappingExportTestDir8": should export the mapping with mapping id "sync/extract_script_test" with extracted scripts', async () => {
        const exportDirectory = "mappingExportTestDir8";
        const CMD = `frodo mapping export -xi sync/extract_script_test -D ${exportDirectory}`;
        await testExport(CMD, env, 'sync', undefined, exportDirectory, false);
    });
    test('"frodo mapping export -i mapping/mappingTest --extract -D mappingExportTestDir9": should export the mapping with mapping id "mapping/mappingTest" with extracted scripts', async () => {
        const exportDirectory = "mappingExportTestDir9";
        const CMD = `frodo mapping export -i mapping/mappingTest --extract -D ${exportDirectory}`;
        await testExport(CMD, env, mappingType, undefined, exportDirectory, false);
    });
});
