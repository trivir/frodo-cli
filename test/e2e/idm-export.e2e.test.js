...
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idm export --all-separate --no-metadata --directory idmTestDir3 --entities-file test/e2e/env/testEntitiesFile.json --env-file test/e2e/env/testEnvFile.env
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idm export -AxD idmTestDir9
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idm export -xi endpoint/extractscripttest -D idmTestDir10
*/
...
describe('frodo idm export', () => {
  ...
  test(`"frodo idm export --all-separate --no-metadata --directory idmTestDir3 --entities-file ${entitiesFile} --env-file ${envFile}": should export all idm config entities to separate files in the "idmTestDir3" directory according to the entity and env files`, async () => {
    const dirName = 'idmTestDir3';
    const CMD = `frodo idm export --all-separate --no-metadata --directory ${dirName} --entities-file ${entitiesFile} --env-file ${envFile}`;
    await testExport(CMD, env, undefined, undefined, dirName, false);
  });
   test(`"frodo idm export -AxD idmTestDir9": should export all idm config entities to separate files and extracted scripts in the "idmTestDir9" directory`, async () => {
    const dirName = 'idmTestDir9';
    const CMD = `frodo idm export -AxD ${dirName}`;
    await testExport(CMD, env, undefined, undefined, dirName, false);
  });
  test('"frodo idm export -xi endpoint/extractscripttest -D idmTestDir10": should export the idm config entity with idm id "endpoint/extractscripttest" with extracted idm scripts', async () => {
    const dirName = 'idmTestDir10';
    const CMD = `frodo idm export -xi endpoint/extractscripttest -D ${dirName}`;
    await testExport(CMD, env, type, undefined, dirName, false);
  });
});
