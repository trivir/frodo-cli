...
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idm export -SN script -D testDir4
...
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idm export --all-separate --sort --directory testDir3 --entities-file test/e2e/env/testEntitiesFile.json --env-file test/e2e/env/testEnvFile.env
*/
...
describe('frodo idm export', () => {
  ...
  test('"frodo idm export -SN script -D testDir4": should export the idm config entity with idm id "script" into the directory testDir4', async () => {
    const dirName = 'testDir4';
    const CMD = `frodo idm export -SN script -D ${dirName}`;
  ...
  test('"frodo idm export --all-separate --sort --directory testDir3 --entities-file test/e2e/env/testEntityFile.json --env-file test/e2e/env/testEnvFile.env": should export all idm config entities to separate files in the "testDir" directory according to the entity and env files', async () => {
    const dirName = 'testDir3';
    const CMD = `frodo idm export --all-separate --sort --directory ${dirName} --entities-file test/e2e/env/testEntitiesFile.json --env-file test/e2e/env/testEnvFile.env`;
    await testExport(CMD, env, undefined, undefined, dirName, false);
  });
});
