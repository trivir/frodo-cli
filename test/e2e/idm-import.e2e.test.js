...
/*
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idm import -N script -D test/e2e/exports/all-separate/raw
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idm import -f test/e2e/exports/all-separate/cloud/global/idm/script.json
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idm import --name script --file test/e2e/exports/all-separate/cloud/global/idm/script.json
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idm import -N script -f script.json -D test/e2e/exports/all-separate/cloud/global/idm
...
const rawDirectory = "test/e2e/exports/all-separate/raw";
const idmScriptConfigFileName = "script.json";
...
describe('frodo idm import', () => {
    test(`"frodo idm import -N script -D ${rawDirectory}": should import the idm config with name 'script' from the directory ${rawDirectory}"`, async () => {
        const CMD = `frodo idm import -N script -D ${rawDirectory}`;
    ...
    test(`"frodo idm import --name script --file ${idmScriptConfigExport}": should import the idm config with name 'script' from the file named '${idmScriptConfigExport}'"`, async () => {
        const CMD = `frodo idm import --name script --file ${idmScriptConfigExport}`;
        const { stdout } = await exec(CMD, env);
        expect(removeAnsiEscapeCodes(stdout)).toMatchSnapshot();
    });

    test(`"frodo idm import -N script -f ${idmScriptConfigFileName} -D ${idmExportDirectory}": should import the idm config with name 'script' from the file named '${idmScriptConfigExport}'"`, async () => {
        const CMD = `frodo idm import -N script -f ${idmScriptConfigFileName} -D ${idmExportDirectory}`;
        const { stdout } = await exec(CMD, env);
        expect(removeAnsiEscapeCodes(stdout)).toMatchSnapshot();
    });
