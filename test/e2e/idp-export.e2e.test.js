...
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idp export -SNi google -D idpExportTestDir1
...
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idp export -SNaD idpExportTestDir2
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idp export -A
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo idp export --all-separate --sort --no-metadata --directory idpExportTestDir3
*/
...
describe('frodo idp export', () => {
  ...
  test('"frodo idp export -SNi google -D idpExportTestDir1": should export the idp provider with idp id "google" into the directory idpExportTestDir1', async () => {
    const exportDirectory = 'idpExportTestDir1';
    const CMD = `frodo idp export -SNi google -D ${exportDirectory}`;
  ...
  test('"frodo idp export -SNaD idpExportTestDir2": should export all idp providers to a single file in the directory idpExportTestDir2', async () => {
    const exportDirectory = 'idpExportTestDir2';
    const CMD = `frodo idp export -SNaD ${exportDirectory}`;
    await testExport(CMD, env, type, undefined, exportDirectory, false);
  });
  ...
  test('"frodo idp export --all-separate --sort --no-metadata --directory idpExportTestDir3": should export all idp providers to separate files in the directory idpExportTestDir3', async () => {
    const exportDirectory = "idpExportTestDir3";
    const CMD = `frodo idp export --all-separate --sort --no-metadata --directory ${exportDirectory}`;
    await testExport(CMD, env, type, undefined, exportDirectory, false);
  });
});
