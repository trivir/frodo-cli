...
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://openam-trivir-webinar.forgeblocks.com frodo config-manager export themes webinar
...
describe('frodo config-manager exports', () => {
  ...
  test('"frodo config-manager export themes: should export the themes in fr-config-manager style"', async () => {
    const dirName = 'webinar';
    const CMD = `frodo config-manager export themes ${dirName}`;
    await testExport(CMD, env, undefined, undefined, dirName, false);
  });
