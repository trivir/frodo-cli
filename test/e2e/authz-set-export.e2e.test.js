...
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo authz set export -SNi test-policy-set -D authzPolicySetExportTestDir1
...
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo authz set export -SNaD authzPolicySetExportTestDir2
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo authz set export -A
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo authz set export --all-separate --sort --no-metadata --directory authzPolicySetExportTestDir3 --no-deps --prereqs
*/
...
describe('frodo authz set export', () => {
    ...
    test('"frodo authz set export -SNi test-policy-set -D authzPolicySetExportTestDir1": should export the policy set with id "test-policy-set" into the directory authzPolicySetExportTestDir1', async () => {
        const exportDirectory = "authzPolicySetExportTestDir1";
        const CMD = `frodo authz set export -SNi test-policy-set -D ${exportDirectory}`;
    ...
    test('"frodo authz set export -SNaD authzPolicySetExportTestDir2": should export all policy sets to a single file in the directory authzPolicySetExportTestDir2', async () => {
        const exportDirectory = "authzPolicySetExportTestDir2";
        const CMD = `frodo authz set export -SNaD ${exportDirectory}`;
        await testExport(CMD, env, type, undefined, exportDirectory, false);
    });
    ...
    test('"frodo authz set export --all-separate --sort --no-metadata --directory authzPolicySetExportTestDir3 --no-deps --prereqs": should export all policy sets from the test-policy-set to separate files in the directory authzPolicySetExportTestDir3 with no dependencies and with prereqs', async () => {
        const exportDirectory = "authzPolicySetExportTestDir3";
        const CMD = `frodo authz set export --all-separate --sort --no-metadata --directory ${exportDirectory} --no-deps --prereqs`;
        await testExport(CMD, env, type, undefined, exportDirectory, false);
    });
});
