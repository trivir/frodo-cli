...
FRODO_MOCK=record FRODO_NO_CACHE=1 FRODO_HOST=https://nightly.gcp.forgeops.com/am frodo config-manager push schedules -n taskscan_activate.json -D test/e2e/exports/fr-config-manager/forgeops -m forgeops
*/
...
test(`"frodo config-manager push schedules -n taskscan_activate.json -D ${allDirectory} -m forgeops": should import a specific schedule by name into forgeops"`, async () => {
    const CMD = `frodo config-manager push schedules -n .taskscan_activate.json -D ${allDirectory} -m forgeops`;
    const { stdout } = await exec(CMD, forgeopsEnv);
    expect(removeAnsiEscapeCodes(stdout)).toMatchSnapshot();
});