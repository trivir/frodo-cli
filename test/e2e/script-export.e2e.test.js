...
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo script export -adD scriptExportTestDir2
...
FRODO_MOCK=record FRODO_HOST=https://openam-frodo-dev.forgeblocks.com/am frodo script export --all-separate --default --extract --directory scriptExportTestDir4
*/
...
describe('frodo script export', () => {
  ...
  test('"frodo script export -adD scriptExportTestDir2": should export all scripts, including default scripts, to a single file in the directory scriptExportTestDir2', async () => {
    const exportDirectory = 'scriptExportTestDir2';
    const CMD = `frodo script export -adD ${exportDirectory}`;
    await testExport(CMD, env, type, undefined, exportDirectory);
  });
  ...
  test('"frodo script export --all-separate --default --extract --directory scriptExportTestDir4": should export all extracted scripts, including default scripts, to separate files in the directory scriptExportTestDir4', async () => {
    const exportDirectory = 'scriptExportTestDir4';
    const CMD = `frodo script export --all-separate --default --extract --directory ${exportDirectory}`;
    await testExport(CMD, env, type, undefined, exportDirectory);
  });
});
