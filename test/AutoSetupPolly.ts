...
const { defaultMatchRequestsBy, filterRecording } = frodo.utils.polly;
...
function getFrodoArgsId(start: number) {
  ...
  if (mode !== MODES.RECORD)
    debugMessage({
      message: `AutoSetupPolly.getFrodoArgsId: argsId=${argsId}`,
      state,
    });
  return argsId;
}
...
function getFrodoCommand() {
  ...
  try {
    if (mode !== MODES.RECORD)
      debugMessage({
        message: `AutoSetupPolly.getFrodoCommand: process.argv=${process.argv}`,
        state,
      });
    if (
    ...
    printMessage({
      message: `AutoSetupPolly.getFrodoCommand: ${error}`,
      type: 'error',
      state,
    });
  ...
  if (mode !== MODES.RECORD)
    debugMessage({
      message: `AutoSetupPolly.getFrodoCommand: cmd=${cmd}`,
      state,
    });
  return cmd;
}

export function autoSetupPolly(matchRequestsBy: RequestMatchers = defaultMatchRequestsBy(false)): Polly {
  ...
  for (const host of FRODO_MOCK_HOSTS) {
    ...
    polly.server.host(host, () => {
      ...
      polly.server
        .any('/am/json/*')
        .recordingName(`${getFrodoCommand()}/am`);
      polly.server
        .any([
      ...
      polly.server.any('/environment/*').recordingName(
        `${getFrodoCommand()}${getFrodoArgValue({ name: '--encoding' })}/environment`
      );
      polly.server
        .any('/keys')
