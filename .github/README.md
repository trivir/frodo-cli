...
Counts use the default `agentic` policy and no tenant connection (`frodo mcp server info --profile <profile>`); the pool behind each profile (the inventory number) is larger than what a policy activates. Internal frodo domains (`state`, `cache`, `factory`) are never exposed by any profile; `--include-domains` and `--exclude-domains` narrow the selection further by top-level domain, and `--include-utils` opts the `utils` domain back in.
...
- **Stateless operation.** The transport keeps no sessions -- there is no session state to hijack, and every request is authenticated on its own merits.
