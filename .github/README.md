...
By default the server binds stdio. The optional HTTP transport (`--transport http`, default `127.0.0.1:6277`) validates the `Host` header against a localhost allow-list (extendable with `--allowed-hosts`; `host.docker.internal` is accepted automatically on non-loopback binds, so a bridge-network container can reach the host without changing its network model) and supports a static bearer token (`--mcp-auth-token <secret>` or `FRODO_MCP_AUTH_TOKEN`) enforced on every MCP request -- binding non-loopback without one refuses to start unless `--allow-unauthenticated` is passed; `GET /health` stays open for liveness probes. Request metadata headers are validated era-conditionally: current-protocol clients get the full header cross-checks, while earlier-era gateways (e.g. LiteLLM's default 2025-11-25 revision) are not 400'd for headers their protocol era never sends.
