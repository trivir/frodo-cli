...
export async function getConfig(
  ...
  const defaultServerPropertiesFiles = jsonFiles.filter((f) =>
    f.path.endsWith('.default.properties.server.json')
  );
  ...
  // Handle server files and properties
  if (
    (serverFiles.length > 0 || defaultServerPropertiesFiles.length > 0) &&
  ...
  for (const f of serverFiles) {
    for (const [id, server] of Object.entries(JSON.parse(f.content).server)) {
      ...
      (exportConfig as FullGlobalExportInterface).server.server[id] =
        server as ServerExportSkeleton;
    }
  }
  // Handle default server properties
  for (const f of defaultServerPropertiesFiles) {
    const property = JSON.parse(f.content);
