...
export function getOneLineDescription(themeObj: ThemeSkeleton): string {
  const description = `[${c.cyan(themeObj._id)}] ${themeObj.name}${
    themeObj.linkedTrees
      ? ' (' + c.cyan(themeObj.linkedTrees.join(', ')) + ')'
      : ''
  }`;
  return description;
}
  ...
  try {
    ...
    if (!long) {
      themeList.forEach((theme) => {
        printMessage(
          `${theme.isDefault ? c.cyan(theme.name) : theme.name}`,
          'data'
        );
      });
    } else {
