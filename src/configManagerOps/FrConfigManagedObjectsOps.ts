...
const { saveJsonToFile, getWorkingDirectory , getFilePath, saveTextToFile,} = frodo.utils;
...
function processManagedObjects(managedObjects, targetDir, name) {
    try {
      managedObjects.forEach((managedObject) => {
        ...
        (Object.entries(managedObject) as [string, ObjectSkeleton][]).forEach(([key, value]) => {
            ...
            extractFrConfigDataToFile(value.source, scriptFilename, `${objectPath}`)
            delete value.source;
          }
        });
  
        if (managedObject.actions) {
          (Object.entries(managedObject.actions) as [string, any]).forEach(([key, value]) => {
            if (value.type && value.type === "text/javascript" && value.source) {
          ...
          ([key, value]) => {
            SCRIPT_HOOKS.forEach((hook) => {
              if (
                ...
                extractFrConfigDataToFile(value[hook].source, scriptFilename, `${objectPath}`)
