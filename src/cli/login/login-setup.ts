...
classic:  "openid"'
    ...
    .addHelpText(
      'after',
      `Usage Examples:\n` +
        `  Shortest form — a host with an already-saved connection profile (identified by a unique substring or profile name), which already remembers its own deployment type, so --type isn't needed either; also picks up the default client id "${DEFAULT_LOGIN_CLIENT_ID}" and a wildcard loopback redirect URI if the target AM supports it:\n` +
