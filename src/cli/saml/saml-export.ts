  ...
  .action(
    ...
    async (host, realm, user, password, options, command) => {
      ...
      if (options.entityId && (await getTokens())) {
        ...
        await exportSaml2ProviderToFile(options.entityId, options.file, options.metadata, options.metadataFile);
      }
      // --all -a
      else if (options.all && (await getTokens())) {
        verboseMessage('Exporting all providers to a single file...');
        await exportSaml2ProvidersToFile(options.file, options.metadata, options.metadataFile);
      }
      // --all-separate -A
      else if (options.allSeparate && (await getTokens())) {
        verboseMessage('Exporting all providers to separate files...');
        await exportSaml2ProvidersToFiles(options.metadata, options.metadataFile);
      }
