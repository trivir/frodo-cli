...
import { abortDirectConfigurationSession } from '../../ops/cloud/EnvDirectConfigurationControlOps';
  ...
  program
    ...
    .addHelpText(
      ...
      `Usage Examples:\n` +
        ...
        c.cyan(`  $ frodo direct-config-session abort ${s.connId}\n`) +
        `  Abort a direct configuration session in JSON format with connection ID:\n` +
        c.cyan(`  $ frodo direct-config-session abort --json ${s.connId}\n`)
    )
    .action(
