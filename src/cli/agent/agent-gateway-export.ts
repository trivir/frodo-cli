    ...
    async (host, realm, user, password, options, command) => {
      ...
      if (await getTokens()) {
        ...
        if (options.agentId) {
          ...
          await exportIdentityGatewayAgentToFile(options.agentId, options.file, options.metadata, options.metadataFile);
        }
        // --all -a
        else if (options.all) {
          verboseMessage(
          ...
          await exportIdentityGatewayAgentsToFile(options.file, options.metadata, options.metadataFile);
        }
        // --all-separate -A
          ...
          await exportIdentityGatewayAgentsToFiles(options.metadata, options.metadataFile);
        }
